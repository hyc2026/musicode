...
from musicode.music.database import *
import musicode.music as mp
...
class chord:
    ...
    def split(self, return_type, get_time=False, sort=False, **args):
        ...
        if get_time and return_type != note:
            ...
            for each in no_time:
                ...
                current.start_time = current_time # 给了start_time
            if sort:
    ...
    def __contains__(self, note1):
        ...
        return note1 in self.same_accidentals().notes # 这地方调用same_accidentals的时候不应该#和b两个mode都用下吗
    ...
    def __setitem__(self, ind, value): # c[ind] = value
        if type(value) == str:
            value = toNote(value)
        if type(ind) != slice:
            if ind > 0:
